...
export function HeaderOperationsTable(props: { items: HeaderOp[]; text: Text["authConsole"]; onChange: (key: string, patch: Partial<HeaderOp>) => void; onRemove: (key: string) => void }) {
  const columns: TableColumnsType<HeaderOp> = [
    { title: props.text.op, dataIndex: "op", width: 104, render: (_, record) => <Select options={[{ label: props.text.set, value: "=" }, { label: "Add", value: "+" }, { label: props.text.remove, value: "-" }]} value={record.op} onChange={(op: HeaderOp["op"]) => props.onChange(record.key, { op })} /> },
