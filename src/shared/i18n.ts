...
const zh = {
  ...
  exchanges: {
    ...
    target: "目标地址", requestBody: "请求正文", responseBody: "响应正文", requestHeaders: "请求头",
    responseHeaders: "响应头", captured: "已捕获",
  },
  authConsole: {
    description: "从结构化表单、Payload JSON 或 Token 任一来源编辑 directive，并同步生成其他格式。",
    structured: "结构化编辑", editableSources: "可编辑输入源", enabled: "启用", headerOps: "Header 操作",
    ...
    exactlyOneSelector: (label: string) => `${label} 必须且只能包含 name 或 glob`, invalidHeaderName: (label: string) => `${label} 不是合法的 Header 名`,
    ...
    tokenPrefix: "Token 必须以 dproxy.11. 开头", tokenPayloadMissing: "Token 缺少 payload", tokenDecodeFailed: "Token payload 解码失败",
...
const en: Text = {
  ...
  exchanges: {
    ...
    target: "Target", requestBody: "Request body", responseBody: "Response body", requestHeaders: "Request headers",
  ...
  authConsole: {
    ...
    structured: "Structured editor", editableSources: "Editable sources", enabled: "Enabled", headerOps: "Header operations",
    ...
    tokenPrefix: "Token must start with dproxy.11.", tokenPayloadMissing: "Token payload is missing", tokenDecodeFailed: "Failed to decode Token payload",
