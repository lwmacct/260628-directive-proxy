...
const zh = {
  ...
  authConsole: {
    description: "完全在浏览器中编辑 dp v18：Inline JSON 直接是 Payload，Remote JSON 只包含 RemoteSpec。",
    ...
    resolverHeaderNoticeBeforeOps: "ops 前固定删除 Authorization 和 Content-Length；未启用“保留代理披露 Header”时，还会删除 Forwarded、Via、X-Forwarded-*、X-Real-IP 等代理披露头。",
    resolverHeaderNoticeAfterOps: "ops 后固定删除 x-dproxy-*、Connection、Proxy-Connection、Keep-Alive、Proxy-Authenticate、Proxy-Authorization、TE、Trailer、Transfer-Encoding 和 Upgrade。",
    resolverHeaderNoticeOverride: "Authorization 可以通过 ops 重新设置；x-dproxy-* 与 hop-by-hop Header 即使由 ops 添加，最终也不会发送。Exact Host 操作会改写 Resolver HTTP Host。",
    enabled: "启用", disabled: "禁用", headerMode: "请求 Header Mode", headerOps: "Header 操作", preserveProxyDisclosure: "保留代理披露 Header",
    add: "添加", side: "作用侧", requestSide: "请求", responseSide: "响应", op: "操作", match: "匹配", selector: "选择器", values: "值", set: "设置", remove: "删除", exact: "精确", valuePlaceholder: "输入值后按 Enter", removeHeaderOp: "删除 Header 操作",
    ...
    remoteSpecOnlyHint: "Remote Token JSON 只允许 RemoteSpec。需要制作远端返回内容时，可切换到 Inline 完成 Payload Builder 并复制 JSON，再切回 Remote；该 Payload 不会嵌入 Remote Token。",
    ...
    tokenJSON: "Token JSON", tokenJSONDescription: (prefix: string) => prefix.endsWith(".inline") ? `${prefix}.<base64url-json> 解码后直接是 Payload。` : `${prefix}.<base64url-json> 解码后只允许 RemoteSpec。`,
    ...
    invalidGlob: (label: string) => `${label} 不是合法的 Glob`, removeHasValues: (label: string) => `${label} del 操作不能包含 values`,
    setNeedsValues: (label: string) => `${label} set/add 操作必须包含 values`, hostValues: (label: string) => `${label} Host 只支持单值 set 或 del`,
...
const en: Text = {
  ...
  authConsole: {
    description: "Edit dp v18 entirely in the browser: Inline JSON is the Payload itself; Remote JSON contains only a RemoteSpec.",
    ...
    resolverHeaderNoticeBeforeOps: "Authorization and Content-Length are always removed before ops. Unless proxy disclosure is preserved, Forwarded, Via, X-Forwarded-*, X-Real-IP, and related disclosure headers are also removed.",
    resolverHeaderNoticeAfterOps: "After ops, x-dproxy-*, Connection, Proxy-Connection, Keep-Alive, Proxy-Authenticate, Proxy-Authorization, TE, Trailer, Transfer-Encoding, and Upgrade are always removed.",
    resolverHeaderNoticeOverride: "Authorization may be set again through ops. x-dproxy-* and hop-by-hop headers are never sent even when added by ops. An exact Host op rewrites the resolver HTTP Host.",
    enabled: "Enabled", disabled: "Disabled", headerMode: "Request Header Mode", headerOps: "Header operations", preserveProxyDisclosure: "Preserve proxy disclosure headers",
    add: "Add", side: "Side", requestSide: "Request", responseSide: "Response", op: "Op", match: "Match", selector: "Selector", values: "Values", set: "Set", remove: "Delete", exact: "Exact", valuePlaceholder: "Type a value and press Enter", removeHeaderOp: "Remove header operation",
    requestModules: "Request-scope Modules", attemptModules: "Attempt-scope Modules",
    ...
    remoteSpecOnlyHint: "Remote Token JSON allows only a RemoteSpec. To author the returned document, switch to Inline, build and copy the Payload JSON, then switch back; that Payload is never embedded in the Remote Token.",
    ...
    tokenJSON: "Token JSON", tokenJSONDescription: (prefix: string) => prefix.endsWith(".inline") ? `${prefix}.<base64url-json> decodes directly to a Payload.` : `${prefix}.<base64url-json> decodes to a RemoteSpec only.`,
    ...
    invalidGlob: (label: string) => `${label} is not a valid glob`, removeHasValues: (label: string) => `${label} cannot contain values for del`,
    setNeedsValues: (label: string) => `${label} requires values for set/add`, hostValues: (label: string) => `${label} Host only supports single-value set or del`,
