...
const zh = {
  ...
  directiveConsole: {
    ...
    metadata: "Metadata", addMetadataField: "添加 Metadata 字段", removeMetadataField: "删除 Metadata 字段", metadataKeyPlaceholder: "user_id", metadataValuePlaceholder: "user-1",
    target: "Target", endpoint: "Endpoint", headers: "Headers", modules: "Modules", recovery: "Recovery", bodyStore: "请求正文接纳", bodyStoreDescription: "覆盖当前实例的正文上限、排队等待、读取超时或 chunk 大小；不填写的字段继承实例默认值。", bodyStoreOverride: "为当前请求覆盖实例默认值", bodyMaxBodyBytes: "正文上限（bytes）", bodyQueueWait: "排队等待", bodyReadTimeout: "读取超时", bodyChunkBytes: "Chunk 大小（bytes）",
    targetMode: "目标 URL 模式", baseURL: "Base URL（拼接入站 path/query）", exactURL: "Exact URL（忽略入站 path/query）", proxyURL: "SOCKS5 Proxy URL",
    remoteUUID: "Remote UUID", generateUUID: "生成 UUID", invalidRemoteUUID: "Remote UUID 必须是合法 UUID", redisKey: "Redis Key", httpResolverURL: "HTTP Resolver URL", redisURL: "Redis URL", filePath: "根目录内文件路径",
    ...
    resolverHeaderNoticeAfterMutations: "mutations 后固定移除 x-dp-*、Connection、Proxy-Connection、Keep-Alive、Proxy-Authenticate、Proxy-Authorization、TE、Trailer、Transfer-Encoding 和 Upgrade。",
    resolverHeaderNoticeOverride: "Authorization 可以通过 mutations 重新设置；x-dp-* 与 hop-by-hop Header 即使由 mutations 添加，最终也不会发送。Exact Host mutation 会改写 Resolver HTTP Host。",
    enabled: "启用", disabled: "禁用", headerMutations: "Header Mutations", preserveProxyDisclosure: "保留代理披露 Header",
    add: "添加", side: "作用侧", requestSide: "请求", responseSide: "响应", action: "动作", match: "匹配", selector: "选择器", values: "值", addAction: "Add", setAction: "Set", delAction: "Del", exact: "精确", valuePlaceholder: "输入值后按 Enter", removeHeaderMutation: "删除 Header Mutation",
    orderedModules: "有序 Modules", orderedModulesDescription: "数组顺序是所有当前活跃 Module 的全局执行顺序；每个 Module 的生命周期由其 Definition 静态声明。",
    moduleName: "Module", moduleConfig: "Config JSON", addModule: "添加 Module", removeModule: "删除 Module", invalidModuleConfig: "Config 必须是有效 JSON",
    recoveryController: "Recovery Controller", recoveryDescription: "Controller、触发条件和预算都属于 Payload。", controller: "Controller",
    controllerURL: "Controller URL", controllerTimeout: "Controller Timeout", controllerHeaders: "Controller Headers", addControllerHeader: "添加 Controller Header", removeControllerHeader: "删除 Controller Header",
    ...
    budget: "Recovery Budget", maxRoundTrips: "最大 Round Trips", maxElapsed: "最大总时长",
    tokenSecret: "HMAC Secret", tokenSecretDescription: "仅在当前浏览器中用于签名与校验 Token，不会发送到服务端。", tokenSecretPlaceholder: "输入 HMAC Secret", tokenSecretRequired: "HMAC Secret 不能为空", tokenAuthenticationFailed: "Token HMAC 校验失败",
...
const en: Text = {
  ...
  directiveConsole: {
    ...
    payloadBuilder: "Payload", httpSpecBuilder: "HTTP RemoteSpec", redisSpecBuilder: "Redis RemoteSpec", fileSpecBuilder: "File RemoteSpec",
    metadata: "Metadata", addMetadataField: "Add metadata field", removeMetadataField: "Remove metadata field", metadataKeyPlaceholder: "user_id", metadataValuePlaceholder: "user-1",
    target: "Target", endpoint: "Endpoint", headers: "Headers", modules: "Modules", recovery: "Recovery", bodyStore: "Request admission", bodyStoreDescription: "Override the instance body limit, queue wait, read timeout, or chunk size. Unset fields inherit instance defaults.", bodyStoreOverride: "Override instance defaults for this request", bodyMaxBodyBytes: "Body limit (bytes)", bodyQueueWait: "Queue wait", bodyReadTimeout: "Read timeout", bodyChunkBytes: "Chunk size (bytes)",
    ...
    remoteUUID: "Remote UUID", generateUUID: "Generate UUID", invalidRemoteUUID: "Remote UUID must be a valid UUID", redisKey: "Redis key", httpResolverURL: "HTTP resolver URL", redisURL: "Redis URL", filePath: "File path under configured root",
    ...
    resolverHeaderNoticeAfterMutations: "After mutations, x-dp-*, Connection, Proxy-Connection, Keep-Alive, Proxy-Authenticate, Proxy-Authorization, TE, Trailer, Transfer-Encoding, and Upgrade are always removed.",
    resolverHeaderNoticeOverride: "Authorization may be set again through mutations. x-dp-* and hop-by-hop headers are never sent even when added by mutations. An exact Host mutation rewrites the resolver HTTP Host.",
    enabled: "Enabled", disabled: "Disabled", headerMutations: "Header mutations", preserveProxyDisclosure: "Preserve proxy disclosure headers",
    add: "Add", side: "Side", requestSide: "Request", responseSide: "Response", action: "Action", match: "Match", selector: "Selector", values: "Values", addAction: "Add", setAction: "Set", delAction: "Del", exact: "Exact", valuePlaceholder: "Type a value and press Enter", removeHeaderMutation: "Remove header mutation",
    orderedModules: "Ordered Modules", orderedModulesDescription: "Array order is global across all active Modules. Each Module Definition declares its lifetime statically.",
    moduleName: "Module", moduleConfig: "Config JSON", addModule: "Add Module", removeModule: "Remove Module", invalidModuleConfig: "Config must be valid JSON",
    ...
    controllerURL: "Controller URL", controllerTimeout: "Controller Timeout", controllerHeaders: "Controller Headers", addControllerHeader: "Add Controller Header", removeControllerHeader: "Remove Controller Header",
    ...
    budget: "Recovery Budget", maxRoundTrips: "Maximum round trips", maxElapsed: "Maximum elapsed time",
    tokenSecret: "HMAC Secret", tokenSecretDescription: "Used only in this browser to sign and verify Tokens; never sent to the server.", tokenSecretPlaceholder: "Enter the HMAC secret", tokenSecretRequired: "HMAC Secret is required", tokenAuthenticationFailed: "Token HMAC verification failed",
