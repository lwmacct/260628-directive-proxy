...
func TestDirectiveCodecEndpoints(t *testing.T) {
	...
	if !strings.HasPrefix(encoded.Token, "dproxy.14.r.") || encoded.Document.Remote == nil ||
	...
	handler.ServeHTTP(decodeResponse, httptest.NewRequest(http.MethodPost, "/directives/decode", bytes.NewReader(decodeBody)))
...
func TestDirectiveCodecRejectsInvalidDocument(t *testing.T) {
	...
	handler.ServeHTTP(response, httptest.NewRequest(http.MethodPost, "/directives/validate", strings.NewReader(`{"kind":"inline","payload":{"target":{}}}`)))
