...
func TestApplyRewriteStripsRuntimeHeaders(t *testing.T) {
	...
	if got := req.Out.Header.Get("M-Runtime-Flag"); got != "" {
		t.Fatalf("expected runtime flag header to be stripped, got %q", got)
	}
	if got := req.Out.Header.Get("M-Runtime-Explicit"); got != "" {
		t.Fatalf("expected explicit runtime header to be stripped, got %q", got)
	}
}
