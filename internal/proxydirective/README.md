...
`proxydirective` 负责解析 `Authorization: Bearer dpx1.<payload>` 中的 v1 directive token，并把它转换成 `proxyplan.Plan`。
...
2. bearer token 必须以 `dpx1.` 开头，否则返回 `proxyplan.ErrInvalidPlan`。
3. `payload_codec.go` 解码 base64url 并使用严格 JSON schema，未知字段会被拒绝。
4. `assemble.go` 对 payload 做一次 normalize，并转换成 `proxyplan.Plan`。
...
- `assemble.go`: payload normalize -> `proxyplan.Plan`
