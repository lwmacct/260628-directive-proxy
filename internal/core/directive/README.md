...
`directive` 负责解析 `Authorization: Bearer dproxy.11.<payload>` 中的 v11 directive token，并把它转换成 `proxy.Plan`。
...
- 将 dproxy family 请求与 control 请求分流，decoder 只接受 `dproxy.11.` 三段格式
- 解码 base64url JSON 并校验 v11 payload schema
- 将 target、proxy、headers 等 payload 字段组装成 `proxy.Plan`

## 处理流程

1. `resolver.go` 读取 `Authorization` bearer token。
2. 非 dproxy family token 返回 `proxy.ErrNoMatch`，由 proxy handler 交给下一个 HTTP handler；dproxy family token 必须是 `dproxy.11.<base64url-json>`，否则返回 `proxy.ErrInvalidDirective`。
...
- `payload_codec.go`: dproxy.11 token 编解码和严格 JSON 解码
