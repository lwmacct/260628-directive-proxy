...
`directive` 负责解析 `Authorization: Bearer dp.20.<inline|remote>.<base64url-json>.<hmac>`，先校验 TokenSecret 的 HMAC，再按需通过 HTTP、Redis 或 File 读取完整 directive，并转换成 `proxy.Plan`。
...
5. `assemble.go` 在 Prepare 阶段将 target 与入站 URL 编译成最终 URL，再生成 `proxy.Plan`；resolver 另行返回来源观测信息。
...
- target 必须且只能包含 `base_url` 或 `exact_url`；Plan 只保存编译后的最终 URL，不携带合成策略。
