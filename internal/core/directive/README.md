...
`directive` 负责解析 `Authorization: Bearer dproxy.12.<kind>.<payload>` 中的 v12 directive token，并把它转换成 `proxy.Plan`。
...
- 将 dproxy family 请求与 control 请求分流，decoder 只接受 `dproxy.12.i/r` 四段格式
- inline 解码 base64url JSON；Redis 引用通过 Store 读取完整 JSON
- 校验 v12 payload schema
- 将 target、proxy、headers 等 payload 字段组装成 `proxy.Plan`

## 处理流程

1. `resolver.go` 读取 `Authorization` bearer token。
2. 非 dproxy family token由 proxy handler 交给下一个 HTTP handler；dproxy family token 必须是 `dproxy.12.i/r.<base64url>`。
3. `payload_codec.go` 解码来源；Redis value 和 inline JSON 使用同一严格 schema，未知字段会被拒绝。
...
- `dproxy.12.` 后的 `i`/`r` 明确区分 inline JSON 和 Redis key。
- Redis value 必须是完整 payload，不做合并、回退或递归引用。
...
- `payload_codec.go`: dproxy.12 i/r token 编解码和严格 JSON 解码
