...
func normalizeProgram(program module.Program, allowRequest, allowAttempt bool) (module.Program, error) {
	if !allowRequest && len(program.Request) > 0 || !allowAttempt && len(program.Attempt) > 0 {
		return module.Program{}, ErrInvalidPayload
	}
	request, err := normalizeModuleSpecs(program.Request)
	if err != nil {
		return module.Program{}, err
	}
	attempt, err := normalizeModuleSpecs(program.Attempt)
	if err != nil {
		return module.Program{}, err
	}
	return module.Program{Request: request, Attempt: attempt}, nil
}

func normalizeModuleSpecs(specs []module.Spec) ([]module.Spec, error) {
	if len(specs) > maxModuleSpecs {
		return nil, ErrInvalidPayload
	}
	result := make([]module.Spec, len(specs))
