...
- `request_id`：调用方生成的 16 随机字节 Base64URL 标识，只用于匿名 retry capability 查找。
- `retry capability`：调用方生成的 32 随机字节 Base64URL secret；服务端只保存绑定 `request_id` 的 SHA-256 verifier。
...
Proxy Retry 是每个代理请求的固有能力。初始请求无需携带 retry identity；只有需要使用 Public retry API 时才携带 `Dproxy-Request-ID` 和 `Dproxy-Retry-Capability`。两者在 observability、resolver 和 upstream 看到请求前即被移除。公开重试端点为：

```http
PUT /api/public/proxy-requests/{request_id}/attempts/{next_attempt}
Authorization: DProxy-Retry <request_id>.<capability>
If-Match: "attempt:<current_attempt>"
```

Control API 使用 `PUT /api/control/proxy-requests/{trace_id}/attempts/{next_attempt}` 和相同的 `If-Match`。每个逻辑请求由单一 coordinator event loop 串行处理响应到达和重试命令；重复 PUT 返回首次接受的结果。POST/PATCH 缺少初始 `Idempotency-Key` 时返回 422。
