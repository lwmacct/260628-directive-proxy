...
本文定义 v20 remote directive 的唯一语义：Remote 只是取得 `Payload` 的方式，不拥有任何独立的执行字段。
...
```text
dp.20.inline.<base64url(Payload JSON)>.<base64url(HMAC-SHA256)>
```

remote token 的第四段解码后直接是 `RemoteSpec`：

```text
dp.20.remote.<base64url(RemoteSpec JSON)>.<base64url(HMAC-SHA256)>
```

HMAC 输入是 token 前四段的原文拼接：`dp.20.<kind>.<base64url-json>`；服务端使用 `server.proxy.directive.token-secret` 计算并以 constant-time compare 校验。secret 不进入 token。
...
RemoteSpec 在 `Prepare` 阶段解引用一次。得到的 Payload 会被严格解码、规范化并编译为当前请求唯一的 `PreparedDirective`，固定包含 Source、HTTP Plan、Program 和 Recovery；各 Attempt 消费相同的编译结果。
...
- 代理披露 header 默认在 mutations 前移除；`x-dproxy-*` 和 hop-by-hop header 在 mutations 后统一移除；
