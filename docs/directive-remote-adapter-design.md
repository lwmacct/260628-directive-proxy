...
RemoteSpec 在 `Prepare` 阶段解引用一次。得到的 Payload 会被严格解码、规范化并编译为当前请求唯一的 `PreparedDirective`，固定包含 Source、HTTP Plan、Program、Recovery 和 Metadata；各 Attempt 消费相同的编译结果。
