...
- 单一有序 `modules` 数组，每项使用 `{module, config}` Module Spec；生命周期由 Program Module Catalog 中的 Definition 声明；
- Recovery Controller 直接使用 `{url, headers?, timeout?}` HTTP callback 参数，不属于 Module Catalog；
...
`side` 是每条 mutation 的必填字段，只允许 `request` 或 `response`；`action` 只允许 `add`、`set` 或 `del`。`add` 至少包含一个 value 并逐项追加，`set` 必须且只能包含一个 value，`del` 不包含 `values` 并删除该名称的全部值。mutations 按数组顺序执行；首条 request `del` + `glob: "*"` 表示清空继承的普通 Header 后重建。`preserve_proxy_disclosure` 只影响 request 基线。
...
- `headers` 直接复用 Payload HeaderPolicy 的 `preserve_proxy_disclosure` 与 `mutations`；
- HTTP RemoteSpec 的 mutation 只允许 `side: request`，因为该策略只改写 resolver 请求头；
- 请求以原入站 Header 为基线；需要从空集合重建时，首条 mutation 使用 `action: del` 与 `glob: "*"`；
...
- mutations 后未设置 User-Agent 时显式抑制 Go transport 的默认 User-Agent；
