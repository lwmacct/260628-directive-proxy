...
remote directive 的目的，是在 token 不适合直接容纳易变 Payload 时，从外部位置取得当前 Attempt 的代理计划。HTTP 和 Redis 都只是读取外部 JSON 的适配器，不拥有独立的 directive 语义。稳定的 `source`、request-scope `program` 和 `recovery` 属于 v18 token envelope，不由远端存储提供。
...
该接口由组合根中的 `directiveRemoteReader` 实现，而不是要求每个具体 adapter 直接实现。组合根按 `RemoteSpec.type` 调用具有最小必要参数的具体 reader：

```text
core/directive.RemoteReader
             ▲
             │ implements
appcmd/server.directiveRemoteReader
             │
...
HTTP reader 需要原始请求来构造受限的 resolver metadata；Redis reader 只需要 context 和 `RemoteSpec`。不得为了方法签名表面一致，向具体 adapter 注入其不使用的依赖。

所有具体 reader 的输出都是裸 `Payload` JSON 的原始字节。adapter 不应：

- 把远端内容解释成另一套业务结构；
- 在适配器内部组装或编译 `proxy.Plan`；
- 把 token envelope 的字段写入或复制进远端 payload；
...
### v18 稳定 envelope 是有意边界

remote token 中的以下字段在 Prepare 阶段只校验一次，并跨全部 Attempt 保持稳定：

- `source`：如何读取远端 Payload；
- `program.request`：跨 Attempt 的 request-scope Module；
- `recovery`：触发器、Controller 与 budget。

远端 Payload 只提供每个 Attempt 可变化的 target、header、proxy 和 `program.attempt`。Resolver 读取 Payload 后把已编译的稳定 Recovery policy 附着到 Plan；这不是字段级 merge，也不允许远端覆盖 token envelope。远端 Payload 如果声明 `program.request` 必须被拒绝。

这个例外是 Recovery 设计的关键：Controller 可以先修改远端 Payload，再返回 `retry`；下一 Attempt 重新读取新的代理计划，同时 Recovery Controller 自身不会因为远端内容被改坏而消失。

具体 HTTP 与 Redis adapter 位于相互独立的包中，不彼此导入。`internal/appcmd/server` 作为组合根创建 adapter、适配 core 统一端口、按 `RemoteSpec.type` 分派，并统一关闭资源。core 只依赖 `RemoteReader` 接口，不依赖具体 adapter。
...
HTTP adapter 不应自行理解返回的 JSON 字段。HTTP 返回的仍然是完整裸 `Payload`，而不是引用、补丁、Recovery 配置或 adapter-specific envelope。
...
```shell
redis-cli JSON.GET 'dproxy:directive:team-a/service-a'
```
...
- 具体 adapter 是否只接收自身实际需要的参数？
- `RemoteSpec` 是否仍然自包含？
- 是否意外引入了服务端后端注册或 token 状态？
- 是否对远端 payload 做了字段级合并、回退、递归引用或 value 缓存？稳定 token Recovery/request program 的应用是否仍保持单向且不可被远端覆盖？
