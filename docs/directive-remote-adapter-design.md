...
remote token -> RemoteSpec -> typed reference -> HTTP/Redis reader ─┘
```
...
core 将 RemoteSpec 编译成只包含合法运行时状态的 typed reference，并通过两个精确端口读取远端内容：
...
core resolver 根据已经严格校验的 `RemoteSpec.type` 调用对应端口。组合根 `internal/appcmd/server` 只注入 HTTP/Redis adapter 并管理连接资源，不参与每次请求的类型分派。core 不依赖具体 adapter。
...
RemoteSpec、完整 endpoint、URL query、Redis URL、认证信息和底层 adapter 错误属于正常观测信息，日志和事件不对这些字段做脱敏。是否记录 token 原文或远端 Payload 内容由具体事件协议决定。
