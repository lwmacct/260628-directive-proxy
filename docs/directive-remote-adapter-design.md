...
- 单一有序 `program` 数组，每项只声明唯一 `module` 和可选 `config`；生命周期由 Definition 静态声明；
...
```json
{
  ...
  "recovery": {
    "controller": {"module": "builtin.recovery.http", "config": {"url": "https://controller.example.com/recovery"}},
