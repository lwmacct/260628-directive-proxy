...
`core/event.Dispatcher` 实现该 provider，负责 `dp.event.v5` Record、单 trace sequence、buffer ownership、有界队列、分片和 Sink。Record 包含 `producer`、`topic`、`record_id`、`trace_id`、`metadata`、`round_trip`、`sequence`、`occurred_at` 和 `data`；Dispatcher 在顶层分别附加系统 trace ID 和完整 directive metadata，因此 producer 不在 topic data 中重复二者。Fluent 默认 tag 前缀为 `dp`。
