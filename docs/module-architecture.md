...
`core/event.Dispatcher` 实现该 provider，负责 `dp.event.v6` Record、单 trace sequence、buffer ownership、有界队列、分片和 Sink。Record 包含 `producer`、`topic`、`trace_id`、`metadata`、`round_trip`、`sequence`、`occurred_at` 和 `data`；`(trace_id, sequence)` 是事件身份。Dispatcher 在顶层附加完整 directive metadata，因此 producer 不在 topic data 中重复这些公共字段。Fluent 默认 tag 前缀为 `dp`。
