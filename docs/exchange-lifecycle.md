...
decode token and resolve canonical Payload
  ...
  -> RequestStarted
  -> DirectivePrepared（一次，包含 source/target/payload digest/metadata）
...
Request Module 在正文读取前收到一次 `DirectivePrepared`。每次 Recovery Attempt 开始时，request Module 与新打开的 attempt Module 都收到 `AttemptStarted`；其中 source、target、payload digest 和 metadata 来自同一个 prepared directive，不会在 Attempt 之间重绑定或报告虚假的变化。
