...
主要 topics 为 `capture.request.*`、`capture.directive.*`、`capture.round_trip.*`、`capture.recovery.*` 和 `capture.response.*`。`capture.directive.prepared` 每个请求只输出一次，data 包含 source、固定 target 和 payload digest；`capture.round_trip.started` 每次 RoundTrip 输出一次并重复这些固定事实，便于独立审计。`dp.event.v6` Record 顶层统一提供 trace ID、单 trace sequence 和完整 directive metadata，不在 Capture topic data 中重复；不存在 per-RoundTrip directive resolve 或 metadata changed topic。
