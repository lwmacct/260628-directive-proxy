...
观测插件和输出插件使用不同接口：前者按 trace 保存解析状态并消费 Signal，后者并发安全地接收不可变 Record。Record 的 `plugin` 字段和健康检查使用配置中的插件实例名称；topic 表示稳定的事件类型。Pipeline 统一负责 Record identity、单 trace sequence、topic routing、队列资源限制、panic containment、输出分片和健康聚合。
